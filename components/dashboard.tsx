...
export function Dashboard() {
  ...
  useEffect(() => {
    ...
    const changed = (event: Event) =>
      setSettings((event as CustomEvent<MonitoringSettings>).detail);
      ...
      if (json.success) {
        const readings: RawReading[] = (json.data ?? []).map(
          (reading: RawReading) => ({
            ...
            voltage: reading.voltage !== null ? Number(reading.voltage) : null,
          }),
        );
  ...
  const isOnline = (reading: RawReading | undefined) => {
    ...
    const diffSeconds =
      (now.getTime() - new Date(reading.recordedAt).getTime()) / 1000;
  ...
  const activeSensorId = activeFloor === "4" ? "TEMP-L4" : "esp32-lantai5";

  const activeTemps = rawReadings
    .filter((reading) => reading.sensorId === activeSensorId)
    .map((reading) => Number(reading.temperature))
    .filter((temperature) => Number.isFinite(temperature));

  const maxTemp = activeTemps.length > 0 ? Math.max(...activeTemps) : null;

  const minTemp = activeTemps.length > 0 ? Math.min(...activeTemps) : null;

  const avgTemp =
    activeTemps.length > 0
      ? activeTemps.reduce((total, temperature) => total + temperature, 0) /
        activeTemps.length
      : null;
      ...
      actions={
        <>
          <span className="hidden items-center gap-2 text-sm text-slate-500 xl:flex">
            ...
            {now
              ? `${fullDate(now)}, ${clock(now.toISOString(), true)} WIB`
              : "Memuat waktu..."}
          </span>
            ...
            <div>
              <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                <Metric
                  ...
                  value={
                    readingL4 ? `${Number(readingL4.temperature)}°C` : "--°C"
                  }
                ...
                <Metric
                  ...
                  value={
                    readingL4?.voltage !== null &&
                    readingL4?.voltage !== undefined
                      ? `${Number(readingL4.voltage).toFixed(1)} V`
                      : "-- V"
                  }
                  detail={
                    readingL4?.voltage
                      ? readingL4.voltage >= 200 && readingL4.voltage <= 240
                        ? "Normal (Stabil)"
                ...
                <Metric
                  ...
                  detail={
                    readingL4
                      ? `Update: ${clock(readingL4.recordedAt, true)}`
                      : "Belum ada data"
                  }
                  valueClassName={
                    onlineL4 ? "text-emerald-600" : "text-rose-600"
                  }
            ...
            <div>
              <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                <Metric
                  ...
                  value={
                    readingL5 ? `${Number(readingL5.temperature)}°C` : "--°C"
                  }
                ...
                <Metric
                  ...
                  valueClassName={
                    onlineL5 ? "text-emerald-600" : "text-rose-600"
                  }
                  iconColor="bg-blue-50 text-blue-500"
                />
                <Metric
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                <Limit
                  ...
                  label="Normal"
                  value={`≤ ${Number(settings.warningTemperature)}°C`}
                />
                <Limit
                  color="bg-amber-400"
                  label="Waspada"
                  value={`${Number(settings.warningTemperature)}–${Number(settings.dangerTemperature)}°C`}
                />
                <Limit
                  color="bg-rose-500"
                  label="Bahaya"
                  value={`≥ ${Number(settings.dangerTemperature)}°C`}
                />
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="divide-y">
                <SummaryRow
                  ...
                  value={maxTemp !== null ? `${maxTemp.toFixed(2)}°C` : "--°C"}
                ...
                <SummaryRow
                  icon={TrendingDown}
                  label="Suhu Terendah"
                  value={minTemp !== null ? `${minTemp.toFixed(2)}°C` : "--°C"}
                ...
                <SummaryRow
                  ...
                  value={avgTemp !== null ? `${avgTemp.toFixed(2)}°C` : "--°C"}
          ...
          <section className="mt-6">
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                <CardTitle className="text-sm font-semibold">
                  5 Pembacaan Terakhir (Semua Lantai)
                </CardTitle>
                <Link
                  href="/riwayat"
                        ...
                        return (
                          <TableRow key={row.id}>
                            ...
                            <TableCell className="text-slate-600">
                              {row.voltage !== null
                                ? `${Number(row.voltage).toFixed(1)} V`
                                : "-- V"}
                            </TableCell>
    ...
    <Card className="border-slate-200/80 shadow-sm">
      <CardContent className="flex min-h-32 items-center gap-4 p-5">
        ...
        <div className="min-w-0">
          ...
          <p className={`truncate text-2xl font-bold ${valueClassName}`}>
            {value}
          </p>
...
function SummaryRow({
  ...
  return (
    <div className="flex items-center gap-4 py-3">
      ...
      <span>
        <small className="text-slate-500 block text-[11px] leading-tight font-medium">
          {label}
        </small>
...
function SystemRow({
  ...
  return (
    <div className="flex items-center py-3 text-xs font-semibold">
      <Icon
        className={`mr-2 size-4 ${online ? "text-emerald-600" : "text-rose-600"}`}
      />
      <span className="text-slate-500">{label}</span>
      <span
        className={`ml-auto ${online ? "text-emerald-600" : "text-rose-600"}`}
      >
